...
//Current Purpose of this main.c: OLED displays
...
int main(void)
{
  ...
  while (1) {
	...
	//IMU: V is a two element array holding whole number and decimal value
	...
	char speed[30];
	sprintf(speed, "X: %d.%d Km/h", V[0], V[1]);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, BB_SYSOFF_Pin|LoRa_nRST_Pin|CS_LoRa_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : BB_SYSOFF_Pin LoRa_nRST_Pin CS_LoRa_Pin */
  GPIO_InitStruct.Pin = BB_SYSOFF_Pin|LoRa_nRST_Pin|CS_LoRa_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);

  /*Configure GPIO pins : session_btn_Pin radio_btn_Pin */
  GPIO_InitStruct.Pin = session_btn_Pin|radio_btn_Pin;
